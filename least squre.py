...
if __name__ == '__main__':
    ...
    print(
        f'coefficient:{coefficient}, intercept:{intercept}, score:{r2_score(y_test, predict_1) * 100}, mse:{mean_squared_error(y_test, predict_1)}')
    print(
        f'coefficeint:{coff},intercept:{inter}, score:{r2_score(y_test, predict_2) * 100}, mse:{mean_squared_error(y_test, predict_2)}')
    print(
        f'coefficeint:{model.coef_},intercept:{model.intercept_}, score:{r2_score(y_test, predict_3) * 100}, mse:{mean_squared_error(y_test, predict_3)}')
